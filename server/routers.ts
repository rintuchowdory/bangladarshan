...
export const appRouter = router({
  ...
  library: router({
    ...
    saveHistory: protectedProcedure.input(z.object({ bookId: z.string().min(1).max(120), chapterId: z.number().int().positive(), progress: z.number().int().min(0).max(100) })).mutation(({ ctx, input }) => saveListeningHistory(ctx.user.id, input)),
